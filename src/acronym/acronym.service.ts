...
export class AcronymService {
  constructor(
    @InjectModel('Acronym') private readonly acronymModel: Model<Acronym>,
  ) {}
  ...
  async findRandom(count: number): Promise<Acronym[]> {
    ...
    const randomIndexes: number[] = getRandomIndexes(
      count,
      totalNumberOfAcronyms,
    );

    if (count !== randomIndexes.length) {
      throw new NotFoundException(
        `Can't find ${count} random acronyms among available in the database`,
      );
    }
