...
export class AcronymController {
  ...
  async findPaginated(
    ...
    try {
      ...
      throw new BadRequestException(
        `The search parameter '${search}' is not in the expected RegExp format`,
      );
    }

    const acronyms = await this.acronymService.findPaginated(
      from,
      limit,
      pattern,
    );
  ...
  @Get(':acronym')
  async findOne(@Param('acronym') acronym: string): Promise<Acronym> {
  ...
  async update(
    ...
    @Body('definition') acronymDefinition: string,
  ): Promise<void> {
  ...
  @Delete(':acronym')
  @HttpCode(204)
  async delete(@Param('acronym') acronym: string): Promise<void> {
